...
function generateResult(data){
    ...
    <div className="jumbotron">
               <div className="row">
                   ...
                   <div style={{borderRadius:'22px'}} style={{alignItems: 'center'}}>
                        <h2>Project Name: &nbsp;{eachElement[1]}                      <button id={eachElement[0]} onClick={() =>{loadDetails(eachElement[0])}} className="btn btn-info">More..</button></h2>
  ...
  ReactDOM.render((Project_Element.length > 0 ? Project_Element:default_result),document.getElementById('Blogs-loadDataBase'))
  ...
  for(let i = 1; i < allContent.length;i++){
    ...
    <div className="jumbotron">
               <div className="row">
                   <div className="col-md-9">
                   <div style={{borderRadius:'22px'}} style={{alignItems: 'center'}}>
                        <h2>Project Name: &nbsp;{eachElement[1]}                      <button id={eachElement[0]} onClick={() =>{currentResult = "Default"; updateAjax();}} className="btn btn-info">Return to the blogs page</button></h2>
                        {console.log(eachElement[0])}
                        <h5>ID:  &nbsp;{eachElement[0]}</h5>
                        <p> {eachElement[3]}</p>
                        <p> {eachElement[4]}</p>
...
function loadDetails(projectId){
  ...
  const data = { id: projectId };
      fetch("update_ajax_detailed_blog.php", {
            method: 'POST',
            body: JSON.stringify(data),
            headers: { 'content-type': 'application/json' }
        })
